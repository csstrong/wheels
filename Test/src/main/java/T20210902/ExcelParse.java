...
public class ExcelParse {
     ...
     *                                          "data_value":{"SO2:"12.22",”SO2-Unit“:”(mg/m3)“，"NO2:"15.22","NO-Unit":"(mg/m3)"}，"storage_time":""}
     */
    //Mavne:导入poi和fastjson依赖
    public List<String> decodeExcel(String deviceId, Map<String, List<String>> factorMap, FileInputStream stream) throws IOException {
        ...
        for (int icount = 1; icount <=rows; icount++) {
            ...
            for (int j = 1; j <cols; j++) {
                ...
                String cName=row0.getCell(index).toString();//获取列名称 "SO2标态干基值(mg/m3)"
                if(list.contains(cName)){//当前单元格的列方向表头数据在list中时组建json数据
                    ...
                    //3-2.放入元素单位
                    int indexLeftBracket=0;//左括号位置
                    for(int m=0;m<cName.length();m++){
                        char c = cName.charAt(m);
                        if(String.valueOf(c).equals("(")){
                            indexLeftBracket=m;
                        }
                    }
                    json.put(mapFlag.get(cName)+"-Unit",cName.substring(indexLeftBracket+1,cName.length()-1));
                }
            }
            resJson.put("data_value",json);//3-3.放入data_value
...
//        factorMap.put("SO2",l);
//
//        String filename = "E:\\Documents\\WeChat Files\\wxid_5fbqy8jyrll222\\FileStorage\\File\\2021-09\\(Min_1)2021_09_02 15_36_14.xls";
